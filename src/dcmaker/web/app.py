...
def api_convert(file: UploadFile = File(...),
    ...
    if preset not in PRESETS:
        raise HTTPException(422, f"preset must be one of {tuple(PRESETS)}")
    ...
    req = ConvertRequest(
        ...
        priority=priority or None, ss=ss or None, to=to or None,
        duration=dur, out_dir=OUT_DIR)
    try:
        r = convert(req, settings)
    ...
    return {
        ...
        "budget_kb": PRESETS[preset].target_kb, "notes": r.notes,
    }
